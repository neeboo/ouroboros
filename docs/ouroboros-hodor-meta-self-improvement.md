...
一支制作团队正在赶一条短片。第 17 个镜头里，角色穿过房门后站错了方向，前后空间关系断了。团队让生成系统重做三次。每次都得到一张新画面，也每次带来新的细节问题。

系统一直在工作，进度条也一直在动。到了下一部片子，相似的空间错误仍可能再次出现。因为这次返工没有改变系统以后处理同类问题的方法。

一个真正具备自进化能力的 Hodor，会走出另一条路径。

它先把这次返工记录成一段经过隐私处理的生产经历。随后，它发现最近二十个复杂场景里，有七个出现了相似的空间连续性问题。它提出一个可以被验证的解释：当前策略在人物跨区域移动时，过早压缩了空间关系。接着，它生成一个候选策略，在隔离的镜头集合里和旧策略使用相同资源进行比较。

候选策略减少了返工，也没有损害其他类型镜头。产品负责人看到证据、成本和风险后，批准它进入小范围观察。观察结束后，这项策略成为 Hodor 下一轮工作的起点。以后再遇到类似场景，系统从已经验证的新方法开始工作。

这里发生的核心变化，是一次经历被系统转化成了可继承的能力。生成更多结果属于执行；从经历中改变未来的工作方式，才进入自进化。

## 自进化的明确定义

**自进化，是系统在人的使命和边界内，从自身经历中持续完成“观察、解释、试变、选择、继承”，让经过验证的改变成为下一代系统的起点。**

这五个动作缺少任何一个，闭环都会断开。

### 1. 观察

系统能看到真实结果：用户是否接受、哪里发生返工、成本多少、哪个步骤卡住、改动带来哪些副作用。经历会被保存为可追溯证据，而不是留在聊天记录或某个人的记忆里。

### 2. 解释

系统需要提出原因。它要说明问题为什么发生、准备改变哪个机制、预期什么会改善，以及什么结果会证明这个判断错了。只有“测试失败了”还不够，它只是一个现象。

### 3. 试变

系统能生成自己的候选版本。变化可以落在一次具体交付，也可以落在以后完成同类工作的提示、工具、记忆、角色分工、流程和评价方式上。候选必须留在明确的可变范围内。

### 4. 选择

新方案要和旧方案公平比较。它们使用相同模型、时间、次数、工具和预算；候选生成过程看不到保留验证集；无关任务也要接受回归检查。选择依赖独立证据，不能依赖系统对自己的评价。

### 5. 继承

通过验证的方案会获得准确版本和应用回执，成为下一轮默认能力。失败方案会被淘汰，异常方案可以回到最后已知良好的版本。没有继承，实验只产生了一份报告；有了继承，系统的起点才真正改变。

可以把它压缩成一条链：

```text
真实经历 → 因果解释 → 受限候选 → 公平选择 → 版本继承 → 新的真实经历
```

这条链能够持续运行，系统才会随着使用逐步改善。

## 什么容易被误认为自进化

| 看起来像进化的行为 | 实际完成了什么 | 还缺什么 |
| --- | --- | --- |
| 失败后自动重试 | 重复执行原来的方法 | 没有解释、候选和选择 |
| 模型重写自己的提示 | 产生了一个新版本 | 没有基线、保留验证和继承证据 |
| 修好当前测试 | 修复了一次具体交付 | 没有证明未来同类任务会长期改善 |
| 定期升级模型 | 外部替换了能力来源 | 系统没有从自身经历完成选择 |
| 代理说新方案更好 | 完成了一次自我评价 | 缺少独立指标和真实结果 |
| 候选胜出后直接上线 | 做出了一次选择 | 缺少授权、观察窗口和回退能力 |

自进化的判断标准很具体：改进来自真实经历；候选改变了未来行为；选择有独立证据；有效改变被系统继承；整个过程可以审计、停止和回退。

## 自进化发生在哪一层

| 改进层次 | 改变什么 | 例子 | Ouroboros 当前范围 |
| --- | --- | --- | --- |
| 结果改进 | 某一次具体交付 | 修正代码、补一份分镜、修复一次任务 | 已广泛支持 |
| 工作系统改进 | 以后完成同类任务的方法 | 调整提示、工具、角色分工、记忆、评价方式和恢复流程 | 当前重点 |
| 模型改进 | 模型权重和训练过程 | 训练或微调基础能力 | 当前不开放 |

修好一个测试属于结果改进。系统进一步说明重复失败的机制，改变后续工作方法，并在保留任务和无关任务上证明长期提升，才形成工作系统层面的自进化。

结果改进解决眼前问题。工作系统改进降低未来一整类问题的发生率和处理成本。这是 Ouroboros 希望持续积累的产品能力。

## Ouroboros 为什么是一个元自进化系统

普通自进化系统学习怎样把自己的工作做好。元自进化系统设计“一个系统应该怎样学习”。

Ouroboros 为目标系统提供六个组成部分：

- **方向。** Founder Charter 说明系统服务谁、追求什么、哪些价值不能牺牲。
- **感知。** 生产经历和策略信号让系统知道真实世界发生了什么。
- **设计。** 因果假设和候选方案让变化有清楚的理由。
- **实验室。** 等预算实验、保留验证集和无关回归负责筛选方案。
- **记忆。** 版本、回执、结果和适用边界让有效经验能够继承。
- **治理。** 人工授权、成本门、停止条件和回退能力限制风险。

对 Hodor，Ouroboros 设计的是影视生产系统如何观察、实验、选择和继承。换成另一个目标系统，同一套内核仍然可以使用，但业务目标、数据、指标和可变范围会由新的项目定义。

Ouroboros 也把自己放进这套机制。它读取自己的运行失败、重复修复、验证结果和集成回执，提出控制面改进，经过独立验证后更新自身。向内，它改进进化内核；向外，它帮助目标系统建立进化能力。

这就是“元”的含义：它交付的是一套会持续改进的工作系统，以及设计这种系统的方法。

## 为什么值得做

今天大多数智能软件都能完成任务，却很难证明自己会持续变好。

常见改进依赖人工发现问题、写需求、组织开发和验收。模型可以加快其中某些步骤，整体仍然以一次次项目交付为单位。修复结束后，原因、适用范围和意外影响经常没有变成下一轮可以直接使用的能力。

长期来看，这会限制改进速度，也会让自主系统的风险随权限一起增长。团队需要一套公共机制，把目标、证据、假设、实验、授权、交付、验收、推广、回滚和复盘连起来。

Ouroboros 解决的是“改进能力本身”的产品问题。产品可以从等待人来修改，逐步走向持续发现机会、受控验证和积累有效能力。
...
以后判断这项事业有没有前进，可以反复问同一个问题：系统是否已经把真实经历变成因果解释，把解释变成受限候选，用独立证据完成选择，并让有效改变进入下一代？这条链走通一次，产生一次可信改进；能够持续走通，才形成真正的自进化。
