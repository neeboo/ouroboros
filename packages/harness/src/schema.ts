export const EVOLUTION_ACTION_RECEIPTS_SCHEMA_SQL = `
  ...
  create unique index if not exists idx_founder_charters_project_id_unique
    on founder_charters(project_id, id);
  create unique index if not exists idx_design_proposals_project_id_charter_unique
    on design_proposals(project_id, id, charter_id);
  create unique index if not exists idx_design_decisions_proposal_id_charter_unique
    on design_decisions(proposal_id, id, charter_id);
  ...
  create table if not exists evolution_action_receipts (
    ...
    design_proposal_id text not null,
    design_decision_id text not null,
    design_charter_id text not null,
    record_kind text not null check (record_kind in ('profile','variant','experiment')),
    ...
    foreign key (project_id, design_charter_id)
      references founder_charters(project_id, id) on delete restrict,
    foreign key (project_id, design_proposal_id, design_charter_id)
      references design_proposals(project_id, id, charter_id) on delete restrict,
    foreign key (design_proposal_id, design_decision_id, design_charter_id)
      references design_decisions(proposal_id, id, charter_id) on delete restrict,
  ...
  create index if not exists idx_evolution_action_receipts_design_source
    on evolution_action_receipts(
      design_proposal_id, design_decision_id, design_charter_id, created_at, action_event_id
    );
