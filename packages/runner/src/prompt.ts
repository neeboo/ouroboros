...
function renderTargetEvolutionProposalContract(role: string, run: PromptInput["run"]): string {
  ...
  return [
    ...
    '- mutationSurfaces[].evolutionTarget must be exactly "artifact" or "harness". The parser knows "model", but milestone-one model mutation is prohibited. Never use "workflow", "prompt", "tool", "policy", or "code" as evolutionTarget.',
    '- mutationSurfaces[].layer is a separate enum: artifact allows layer "artifact", "code", or "policy"; harness allows layer "workflow", "prompt", "tool", "policy", or "code". Do not invent delivery or product-area layer names.',
    "- After changing any pack field, recompute maturityGateContract.packRef.contentSha256 from the final normalized evolutionPack using the production canonical hash. Never copy the example pack hash after changing the pack; the parser requires packRef id, version, and hash to match exactly.",
    ...
    "- Emit exactly one proposal.evaluationContract object. Merge comparison beside successMetrics and requiredEvidence in that same object. Never place evaluationContract inside evolutionPack and never emit a second evaluationContract that can overwrite the frozen success contract.",
    "- proposal.evidenceRefs is authority input, not a bibliography. It must contain only durable active strategy signal IDs from this target project. Do not put proposal IDs, decision IDs, Git SHAs, or free-form evidence labels there; keep those references inside the strategy signal evidence or proposal narrative.",
